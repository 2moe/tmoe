...
remove_tmoe_linux_tool(){ 
	cd /usr/local/bin
	echo "${RED}rm -rv /usr/share/applications/tmoe-linux.desktop ${HOME}/.config/tmoe-linux /usr/local/etc/tmoe-linux startvnc stopvnc debian-i startx11vnc startxsdl x11vncpasswd .tmoe-linux-qemu startqemu${RESET}"
	DEPENDENCIES='git aria2 pv wget curl less xz-utils newt whiptail'
	echo "${RED}${PACKAGES_REMOVE_COMMAND} ${DEPENDENCIES}${RESET}"
	do_you_want_to_continue
 	rm -rfv /usr/share/applications/tmoe-linux.desktop ${HOME}/.config/ttmoe-linux /usr/local/etc/tmoe-linux startvnc stopvnc debian-i startx11vnc startxsdl x11vncpasswd
...
install_docker_ce_or_io(){
	if [ -e "/tmp/.Tmoe-Proot-Container-Detection-File" ]; then
		echo "${RED}WARNING！${RESET}检测到您当前处于${GREEN}proot容器${RESET}环境下！"
		echo "若您的宿主机为${BOLD}Android${RESET}系统，且处于容器环境下，则请在安装前${BLUE}确保${RESET}您的Linux内核支持docker"
		echo "否则请直接退出安装！！！或者通过qemu-system来运行GNU/Linux虚拟机，再安装docker。"
