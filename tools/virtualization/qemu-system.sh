...
start_tmoe_qemu_aarch64_manager() {
	...
	VIRTUAL_TECH=$(
		whiptail --title "aarch64 qemu虚拟机管理器" --menu "qemu-system virtual machine tmoe manager" 0 50 0 \
...
start_tmoe_qemu_manager() {
	...
	cd /usr/local/bin/
	VIRTUAL_TECH=$(
		whiptail --title "x86_64 qemu虚拟机管理器" --menu "qemu-system virtual machine tmoe manager" 0 50 0 \
			"1" "🍹 Creat a new VM 新建虚拟机" \
