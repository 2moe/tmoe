...
本次更新可以說是天萌自發展以來變化最大的一次，也是難度最大的一次。  
...
簡單來說，為了同步插件管理器的配置，開發者將會為 zsh-i 寫一個配置同步功能 ~~,tmm 只是順帶支持此功能而已。QaQ~~
...
簡單來說，就是先爬取網站數據，再對數據進行分析，最後提取出數據。  
動態獲取存在很多弊端，比如說分析可能會出錯，鏈接可能會失效，維護起來可能會不開心 (ಥ﹏ಥ)。  
~~錘桌！！！（好孩子不要學）~~  
...
在您完成上面的閱讀之後，不妨去思考一下，在天萌的 2022 版中，超過一週時間沒有更新索引數據會發生什麼事。
...
txt 文件裡面包含了安裝過程中需要的一些依賴信息。  
有些功能在沒有相關依賴的情況下是不能運行的。  
舉個例子：  
比如說您要運行 zsh，所以您裝了 zsh。  
您想要某個插件正常運行，所以您裝了個某個插件的相關依賴。  
在沒有外部依賴的情況下，有兩種解決方法，一是自己手動實現，二是不啟用相關功能。  
舊版的依賴確實很多，關於這點我要向所有天萌的用戶道歉。

新版就不會這樣子了。  
比如說：`git` 依賴  
~~開發者可以調用 `git2-rs` 來手動實現一個簡化版的 `git`的客戶端，這樣子就不需要安裝原系統的 git 了。~~  
