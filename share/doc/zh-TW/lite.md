...
如果需要將其暴露到公網的話，那麼不建議對 novnc 容器使用 `-p` 引數，建議走 nginx 的 443 埠。  
請新建一個網路，將其與 nginx 置於同一網路，並設定 `network-alias`(網路別名), 然後用 nginx 給它加上一層認證（例如`auth_basic_user_file pw_file;`）, 最後將其 reverse proxy 出去。  
注：proxy_pass 那裡要寫 `http://novnc容器的網路別名:36080` 。  
如果 nginx 那裡套了 tls 證書，那麼訪問地址就是 `https://您的novnc的域名:埠`。  
