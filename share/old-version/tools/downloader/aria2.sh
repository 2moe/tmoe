...
upgrade_tmoe_aria2_tool() {
    cd /usr/local/bin
    curl -Lv -o aria2-i 'https://raw.githubusercontent.com/2moe/tmoe-linux/master/share/old-version/tools/downloader/aria2.sh'
...
tmoe_aria2_onekey() {
    ...
    aria2c --console-log-level=warn --no-conf --allow-overwrite=true -o aria2.conf 'https://raw.githubusercontent.com/2moe/tmoe-linux/master/share/old-version/.config/aria2.conf'
    case ${TMOE_PROOT} in
...
exit 0
    ...
    cd ${TMOE_ARIA2_PATH}
    if [ ! -e "dht.dat" ]; then
        aria2c --console-log-level=warn --no-conf --allow-overwrite=true -o dht.dat https://raw.githubusercontent.com/2moe/tmoe-linux/master/share/old-version/.config/dht.dat
        chmod 666 dht.dat
    fi
    if [ ! -e "dht6.dat" ]; then
        aria2c --console-log-level=warn --no-conf --allow-overwrite=true -o dht6.dat https://raw.githubusercontent.com/2moe/tmoe-linux/master/share/old-version/.config/dht6.dat
        chmod 666 dht6.dat
    fi
...
create_aria_ng_desktop_link() {
    ...
    if [ ! -e "/usr/share/icons/ariang.png" ]; then
        if [ -e "${ARIA_NG_ICON}" ]; then
            ...
            aria2c --console-log-level=warn --no-conf --allow-overwrite=true -d /usr/share/icons -o ariang.png https://raw.githubusercontent.com/2moe/tmoe-linux/master/share/icons/ariang.png
        fi
    fi
