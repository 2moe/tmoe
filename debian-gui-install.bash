...
DEBIANMENU() {
	cd ${cur}
	OPTION=$(
		whiptail --title "Tmoe-linux Tool输debian-i启动(20200331-18)" --menu "Type 'debian-i' to start this tool.Please use the enter and arrow keys to operate.当前主菜单有十几个选项，请使用方向键或触屏上下滑动，按回车键确认。0326本次更新在软件商店中加入了度盘和云音乐。0331优化WSL2" 19 50 7 \
	...
	cat >startxsdl <<-'EndOfFile'
		...
		if [ "$(uname -r | cut -d '-' -f 3)" = "Microsoft" ] || [ "$(uname -r | cut -d '-' -f 2)" = "microsoft" ]; then
			...
			if grep -q '172..*1' "/etc/resolv.conf" || grep -q '192..*1' "/etc/resolv.conf"; then
		        echo "检测到您当前使用的可能是WSL2，如需手动启动，请在xlaunch.exe中勾选勾选Disable access control"
